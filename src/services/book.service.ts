...
import { ILogger, NotFoundItemError } from 'common-services';
...
export class BookService implements IBookService {
  constructor(@inject(TYPES.PrismaClient) private prismaClient: PrismaClient, @inject(TYPES.ILogger) private logger: ILogger) {}
  ...
  async getById(id: number): Promise<BookModel> {
    ...
    if (!book) {
      throw new NotFoundItemError(`Not Found Book with id ${id}`);
    }
  ...
  async createBook(bookCreateDto: BookCreateUpdateDto): Promise<BookModel> {
    ...
    if (!categoryRef) {
      throw new NotFoundItemError(`Not Found Category with Id: ${bookCreateDto.categoryId}`);
    }
  ...
  async updateBook(
    ...
    if (!categoryRef) {
      throw new NotFoundItemError(`Not Found Category with id: ${bookUpdateDto.categoryId}`);
    }
